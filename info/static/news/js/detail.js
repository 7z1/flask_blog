...
$(function(){
    ...
    // 文章点赞
    $('.diggit').delegate('a','click',function(){

        var sHandler = $(this).prop('class');
        var $this = $(this);
        var action = "add"
        // 用一个类名comment_up_action来控制当前次数是否点赞，
        if(sHandler.indexOf('comment_up_action')>=0)
            {
                // 如果当前该新闻已经是点赞状态，再次点击会进行到此代码块内，代表要取消点赞
                action = "remove"
            }
            ...
            success: function (resp) {
                if (resp.errno == "0") {
                    ...
                    if (action == "add") {
                        ...
                        $this.addClass('comment_up_action')
                    }else {
                        like_count = parseInt(like_count) - 1
                        $this.removeClass('comment_up_action')
                    }
                    // 更新点赞数据
